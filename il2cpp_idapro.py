...
class ScriptContext:
	...
	def process(self):
		...
		try:
			...
			metadata_path = os.path.join(self._backend.get_script_directory(), "./il2cpp_idapro.json")
...
class IDADisassemblerInterface(BaseDisassemblerInterface):
	...
	def on_start(self):
		...
		if IDACLANG_AVAILABLE:
			header_path = os.path.join(self.get_script_directory(), "./il2cpp_idapro.h")
			...
			ida_typeinf.idc_parse_types(os.path.join(self.get_script_directory(), "./il2cpp_idapro.h"), ida_typeinf.PT_FILE)
